...
def index():
    ...
    def _reading_books():
        return Book.query.filter(Book.user_books.any(status='reading')).limit(12).all()

    @cache.cached(timeout=300, key_prefix='read_books_12')
    def _read_books():
        return Book.query.filter(Book.user_books.any(status='read')).limit(12).all()

    @cache.cached(timeout=300, key_prefix='wish_books_12')
    def _wish_books():
        return Book.query.filter(Book.user_books.any(status='wish')).limit(12).all()
